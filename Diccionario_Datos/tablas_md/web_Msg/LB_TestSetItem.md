...
| DeltaCheckCLastResultInterval | varchar |  |  | SI | Interval to previous result for Delta Check |
| DeltaCheckLastTestSetItem_DR | varchar |  | FK | SI | previous LBTSI Link for Delta Check |
...
| HTMLTextResult | longvarchar |  |  | SI | Temporary stream that stores the text result until... |
| ID | varchar |  |  | NO | - |
| Interpretation_DR | bigint |  | FK | SI | Temporary storage to keep the origial range interp... |
...
| LBTSI_ParRef | bigint |  |  | SI | Parent TestSet DR
Required by User.LBTestSetItem. |
...
| LBTestSetBloodProductMsg_DR | varchar |  | FK | SI | - |
| LBTestSetMsg_DR | varchar |  | FK | SI | - |
| PreviousResults | varchar |  |  | SI | Temporary custom link to previous results |
| ReadOnly | bit |  |  | SI | - |
| SessionId | varchar |  |  | SI | - |
| TempAutoCommentsExecuted | varchar |  |  | SI | Temp Stores Rules executed ( only until child get ... |
| TestItemDeltaChecks_DR | varchar |  | FK | SI | Temporary custom link to test item delta checks |
| TestItemRanges_DR | varchar |  | FK | SI | Temporary custom link to the test items ranges |
| childsub | bigint |  |  | NO | - |
